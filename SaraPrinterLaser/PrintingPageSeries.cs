...
namespace SaraPrinterLaser
{
    public partial class PrintingPageSeries : Form
    {
        ...
        Dictionary<string, string> TopPictureModel = new Dictionary<string, string>();
        Dictionary<string, string> BottomPictureModel = new Dictionary<string, string>();
        ...
        private void PrintingPageSeries_Load(object sender, EventArgs e)
        {
            ...
            if (PrintAttrib.SelectData)
            {
                ...
                if (!NoPrintAvabile)
                {
                                    ...
                                    if (item.Key.Contains("روی کارت"))
                                    {
                                        ...
                                        if (PICBX.Name == CntrlName)
                                        {
                                            try
                                            {
                                                ...
                                                TopPictureModel.Add(PICBX.Name, PathAdddresss);
                                    ...
                                    else if (item.Key.Contains("زیر کارت"))
                                    {
                                        ...
                                        if (PICBX.Name == CntrlName)
                                        {
                                            try
                                            {
                                                PICBX.SizeMode = PictureBoxSizeMode.Normal;
                                                string PathAdddresss = PictureFolderPath + "\\" + dataget[answerposition] + PictureFormat;
                                                BottomPictureModel.Add(PICBX.Name, PathAdddresss);
                                                Bitmap bmp = new Bitmap(PathAdddresss);
                    ...
                    if (!flgFailFolder)
                    {
                        if (TopPictureModel.Count != 0 || BottomPictureModel.Count != 0)
                        {
                            putPicturesToEZD(TopPanel, TopPictureModel, WhichPanel.enumTopPanel);
                            TopPictureModel.Clear();
                            putPicturesToEZD(BottomPanel, BottomPictureModel, WhichPanel.enumBottomPanel);
                            BottomPictureModel.Clear();
        ...
        private static PicutreToEzdStatus putPicturesToEZD(Panel WhichPanel, Dictionary<string, string> Pictures, WhichPanel _whichPanel)
        {
                ...
                foreach (Control item in WhichPanel.Controls)
                {
                    ...
                    if (item.GetType().ToString() == "System.Windows.Forms.PictureBox")
                    {
                        ...
                        if (Pictures.TryGetValue(tmp.Name, out answerposition))
                        {
                            ...
                            string BmpName = "";
                            if (_whichPanel == PrintingPageSeries.WhichPanel.enumTopPanel)
                                BmpName = Path.GetTempPath() + "LaserPrinterImagetmp\\" + "PictureTop_" + tmp.Name.Replace(".txt", "");

                            else if (_whichPanel == PrintingPageSeries.WhichPanel.enumBottomPanel)
                                BmpName = Path.GetTempPath() + "LaserPrinterImagetmp\\" + "PictureBottom_" + tmp.Name.Replace(".txt", "");
                            ...
                            */
                            ...
                            if (TmpStatus == Laser.LmcErrCode.LMC1_ERR_SUCCESS)
                            {
                                //TmpStatus = Laser.AddFileToLib(BmpName, "Picture" + EntityName.ToString(), NewCentreLocationX, NewCentreLocationY, 0, 6, 1, 3, false);
                                TmpStatus = Laser.AddFileToLib(BmpName, "Picture" + EntityName.ToString(), NewCentreLocationX, NewCentreLocationY, 0, 6, LaserSetting.Ratio, 3, false);
                                if (TmpStatus == Laser.LmcErrCode.LMC1_ERR_SUCCESS)
                                {
                                    //  TmpStatus = Laser.SetBitmapEntParam("Picture" + EntityName.ToString(), BmpName, 0, 0, 0, 0, 0, 600, false, 0);
                                    TmpStatus = Laser.SetBitmapEntParam("Picture" + EntityName.ToString(), BmpName, 0, 0, LaserSetting.Brightness, LaserSetting.Contrast, LaserSetting.PointTime, LaserSetting.dpi, false, 0);
                                    if (TmpStatus == Laser.LmcErrCode.LMC1_ERR_SUCCESS)
                                        EntityName++;
                                    else
                                    {
        ...
        Bitmap[] SplitPictureControl(Panel Ctrl)
        {
            ...
            while (TwoType[0] == null && TwoType[1] == null);
